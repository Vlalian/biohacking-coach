...
describe('synthetic fixtures — determinism', () => {
  for (const state of SYNTHETIC_STATES) {
    it(`same state + same reference date → identical dataset (${state})`, () => {
      expect(syntheticDataset(state, TODAY)).toEqual(syntheticDataset(state, TODAY));
    });
  }
});

describe('synthetic fixtures — shape invariants', () => {
  ...
  it('done sessions have Body and Mind Feedback in RPE range; skipped have none', () => {
    for (const s of rich.sessions) {
      if (s.status === 'done') {
        expect(s.body).toBeGreaterThanOrEqual(1);
        expect(s.body).toBeLessThanOrEqual(10);
        expect(s.mind).toBeGreaterThanOrEqual(1);
        expect(s.mind).toBeLessThanOrEqual(10);
      } else {
