...
- **→ Design position (2026-07-16, eval-MVP scope grill):** every change you make to an athlete's plan is narrated to the athlete by the AI Coach *with attribution* ("your coach added an interval session Thursday — here's how it fits") at the athlete's next natural touchpoint — no silent plan edits, ever. Is that the right default from the coach's chair? Would you sometimes rather announce a big change yourself and leave the AI only the routine ones — and is there any change you'd want to make *without* immediate narration (e.g., quietly restructuring a future week before it's final)?
