...
- **Round-2 expert interview completed (2026-07-12, athlete expert, live in-chat grill session in Danish).** Sections 9–11 of the interview guide. Findings:
  - **OVERTURNED — Rest dominance is too absolute.** Mobility never counts as load on a rest day (already modelled correctly). But light training (swim, easy run) on recovery days is normal for elites; "one true rest day per week" applies to age-groupers, not everyone. Rest protection should be level-dependent. Affects Displacement rules and ADR-0002; redesign decision pending.
  - **OVERTURNED (possibly delete the feature) — Cross-Week Move.** Expert has never moved a session to the next week: "a missed session is just a session you didn't do that week"; the program effectively restarts each week, and catch-up belongs in the Weekly Session conversation. The Move Checkpoint warning solves a problem no athlete may have. Validate with the human coach (section 12), then decide whether to drop Cross-Week Move + Move Checkpoint entirely.
  - **OVERTURNED — no in-app athlete↔coach chat.** Expert: "Alt skal jo samles der så man ikke har kontakt flere steder" — the whole point is consolidating the coach relationship in one place. Challenges the Coached Mode scope decision (ADR-0003). Revisit for V1.
  - **REVISED — nutrition scope is both, not either/or.** Computed fueling targets as the core, plus *optional* food logging as a self-check tool ("er jeg ved at få det fuldt?"). Not mandatory tracking. Requires a food database eventually (USDA FoodData Central / OpenFoodFacts). Fueling priority: carbs during training + race-week eating first (where the expert himself lacks experience and once hit the wall pre-coaching); daily macro targets secondary.
  - **VALIDATED — weekly rhythm.** His real coaching contact: one weekly status message + video call when needed. Maps directly to Weekly Session + Coach Chat.
  - **VALIDATED — never block, always assess.** Ordinary athletes train suboptimally all the time and that's fine; the app's job is continuous load/injury-risk assessment, not blocking moves. Consecutive run days = a pattern to watch over weeks, never an in-the-moment block.
  - **VALIDATED — strength is athlete territory**, with Coach *suggestions* framed as "for those who want extra".
  - **NEW — sessions must be prescribed in numbers and zones** (watts, pace, HR); natural language is a supplement, never a replacement, regardless of experience level. (The no-raw-numbers rule remains correct for *check-in state data* — this is about workout prescriptions.)
  - **NEW — zones come from field tests** (10+20 min bike/run, CSS swim), and re-tests are triggered by "training feels too easy", not by calendar. The app can detect this signal itself: consistently low Body Feedback on sessions that should be hard → suggest a test. Tests are themselves heavy load — don't prompt too often.
  - **NEW — retro-logging window removed.** No artificial deadline on back-filling sessions. In practice logging is automatic via watch/bike-computer sync — reinforces Garmin integration as the V2 priority.
  - **REVISED — privacy sharing defaults.** Expert would share everything with his coach including AI transcripts ("ens coach er der også for ens mentale helbred") and would change nothing about how he talks to the AI. Keep private-by-default for GDPR and other athlete profiles, but make sharing frictionless and actively suggested at link creation.
  - **Deferred to round 3 (human coach, recruited 2026-07-12):** pattern-reading as a coach, coach-side tools/metrics/workflow, dangerous stacking patterns, AI-drafts-human-corrects from the coach's chair, adoption drivers, never-automate calculations, fueling rules of thumb. Compiled as section 12 of the interview guide.
