...
**Deferred to a second pass** — ~~the two decisions still unresolved on the route map: the **GDPR posture** (lawful basis, consent artifact, Art. 9 health data, deletion path) and~~ the **security hardening consolidation** (CORS, rate limiting, security headers, FIT/GPX metadata sanitisation). ~~Both contain~~ It contains build work; ~~neither is~~ it is not ticketed yet. ~~Their slices join~~ Its slice joins this directory once ~~they lock~~ it locks.

**GDPR posture locked 2026-07-17** ([route ticket 09](../coach-eval-mvp-route/issues/09-gdpr-posture.md)): Article 9 posture on explicit consent; `inference_geo: "us"` workspace-enforced; ZDR skipped; consent in-app and server-recorded with versioned text. Its build work is **[slice 15](issues/15-consent-and-lawful-basis.md)** — which, despite its number, **lands before [slice 06](issues/06-garmin-upload-lands-real-data.md)**: numbers here are filing IDs, and 15 was filed after the original 01–14 sequencing precisely because this decision was deferred then. **Security hardening remains open and gates slice 06 jointly with it** — so slice 06 is still blocked, on one decision now rather than two.
