...
~~**Deferred to a second pass** — the two decisions still unresolved on the route map: the **GDPR posture** and the **security hardening consolidation**. Both contain build work; neither is ticketed yet. Their slices join this directory once they lock.~~

**Nothing is deferred any more — both locked 2026-07-17**, and both now have slices. Numbers here are filing IDs, not order: 15 and 16 were filed after the original 01–14 sequencing precisely *because* these decisions were deferred then, so they arrive last-numbered and early-ordered.

- **GDPR posture** ([route 09](../coach-eval-mvp-route/issues/09-gdpr-posture.md)) — Article 9 on explicit consent; `inference_geo: "us"` workspace-enforced; ZDR skipped; consent in-app, server-recorded, text versioned. → **[slice 15](issues/15-consent-and-lawful-basis.md)**, gating **[slice 06](issues/06-garmin-upload-lands-real-data.md)** (real data).
- **Tier-1 security hardening** ([route 10](../coach-eval-mvp-route/issues/10-security-hardening.md)) — registration disabled on the deployment; Anthropic-side spend and rate limits, no app-level limiter; FIT/GPX file text is data, never instructions. → **[slice 16](issues/16-tier-1-hardening.md)**, gating **[slice 03](issues/03-deploy-to-vercel.md)** (public reachability).

~~**Deferring those decisions gates real data, not the port.** The slices below may be built and demoed against synthetic or otherwise non-production data while the two decisions are open — that is the whole point of sequencing them now.~~ **Both decisions are now made (2026-07-17), so the gate is no longer a decision — it is the two slices that implement them.** The rule is otherwise unchanged: **Mads's real Garmin data must not land on hosted infrastructure, and the eval must not be deployed for the recruited coach, until those controls are in place.** Slice 06 is where that line is crossed first (its notes already require both DPAs concluded); slice 11 is where a second real person arrives. Neither ships ahead of **[slice 15](issues/15-consent-and-lawful-basis.md)**.

**AMENDED 2026-07-17** ([route ticket 10](../coach-eval-mvp-route/issues/10-security-hardening.md)). The paragraph above is right about *real data* and wrong about *when hardening starts to matter*. **Public reachability begins at [slice 03](issues/03-deploy-to-vercel.md), not 06.** [Slice 02](issues/02-login-with-better-auth.md) mints an athlete row on signup and explicitly parks "is registration reachable on the public deployment" with the security-hardening item; slice 03 then deploys that signup route to a public URL and never picks the question up. The data is synthetic at that point and the stakes are low — but the door is open and nothing downstream closes it. **Ticket 10 gates slice 03.** The real-data and coach-access gates at 06 and 11 stand unchanged.

**Not a port.** [ADR 0005](../../docs/adr/0005-nextjs-better-auth-neon-stack.md) lists "the deterministic calc module" among the survivors, but no such module exists in `poc/` — `rules.js` is the Move rules matrix and `infodata.js` is a seeded synthetic-data provider. The calc module is planned new construction with vetted MIT sources to mine, not code to carry across. ~~ADR 0005 wants correcting on this point.~~ **Corrected 2026-07-17** ([route ticket 08](../coach-eval-mvp-route/issues/08-adr-0005-names-a-module-that-does-not-exist.md)): ADR 0005's Consequences now name the three real survivors — prompt rendering, Garmin parsing, and the Move rules.
