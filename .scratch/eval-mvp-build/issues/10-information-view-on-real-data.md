...
`info_layout` JSONB on `athlete` persists favourites, order, and range — already in the signed-off schema.
...
- [ ] Favourites, ordering, and range persist to `info_layout` and survive a refresh
