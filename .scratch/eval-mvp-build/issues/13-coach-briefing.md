...
Link Visibility gates what the Briefing may draw on. If `share_ai_transcripts` is false, the athlete's Coach conversations are not the Briefing's material — the gate is server-side and it applies to what feeds a prompt, not just to what renders. A Briefing that quietly summarises transcripts the coach is not permitted to read defeats the toggle.
...
- [ ] Tests cover persistence and the visibility gate on prompt material
