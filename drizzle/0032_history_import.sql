CREATE TABLE "history_import" (
	...
	CONSTRAINT "history_import_status_known" CHECK (status IN ('uploading', 'unpacking', 'importing', 'done', 'failed'))
);
...
CREATE UNIQUE INDEX "history_import_one_running" ON "history_import" USING btree ("athlete_id") WHERE status IN ('uploading', 'unpacking', 'importing');