...
## Amendment (2026-09-14): the Head Coach owns the Weekly Session Day while linked

**Context.** Automatic Week Plan generation (`training-architecture/16`) delivers next week's proposed plan on the athlete's Weekly Session Day, and a linked Head Coach previews it one day before (`/17`). That makes the day a planning decision rather than a scheduling preference — and the Head Coach is the one doing the planning work that day governs. Until now the day was purely the athlete's, chosen at onboarding, and the three tiers above say nothing about a coach owning an athlete *preference*.

**Decision (Mads, 2026-09-14, `/grill-with-docs`).** One field, and whoever is present writes it. While a Coaching Link is active, **only the Head Coach may change the athlete's Weekly Session Day**; the athlete's Settings control goes read-only. With no link, the athlete edits it as before. The value never resets: severing a link leaves the last-written day in place until the athlete changes it. A coach's change is narrated to the athlete like every other Head Coach action.

**Why this and not the alternatives.** Keeping the athlete's choice and letting the coach override it means two stored values and every reader deciding which wins — the appear/disappear cases become code paths. Removing the athlete's choice altogether and using a default is simpler still, but deletes a working onboarding question to avoid a problem that one ownership rule already avoids. One field with a presence rule is the smallest thing that is correct in both directions.

**Consequences.** "Flexible" is retired — a proposal has to land on *some* day, and "one day before Flexible" is nothing; existing Flexible and unset profiles read as Sunday. Any weekday may be chosen, not only the onboarding shortlist. A draft already in flight when a link changes is not held for a new coach; a draft severed mid-preview is discarded and regenerated, because nobody can tell what the departed coach had half-done to it. This is the first athlete preference the Head Coach outranks the athlete on; it is deliberately the only one, and the reason — *the coach does the work this setting schedules* — is the test any future one has to pass.
