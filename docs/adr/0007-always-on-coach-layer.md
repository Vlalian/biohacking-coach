...
## Amendment (2026-09-16): the Weekly Session is retired as a behavior; the one conversation may propose a week

**Context.** Decision 3 above named four behaviors inside the one thread — Coach Chat, the Weekly Session, Session Negotiation, the Reflective Prompt — and made the Weekly Session the place where two things happened once a week: the athlete reported current form (the Check-in) and the Coach planned the week ahead. Both jobs have since moved. `training-architecture/16` drafts next week silently, as a proposal the athlete accepts, discusses or declines from the calendar (`/18`); the Check-in exists as its own artifact (`/05`). What the Weekly Session still did — reviewing a week and changing it by talking — is what Coach Chat already does for everything else.

**Decision (Mads, 2026-09-16, `/grill-with-docs`).** The Weekly Session stops being a behavior the athlete enters. There is one Coach conversation, athlete-led, about whatever they want — a session, the whole week, their numbers, triathlon. When talk turns into a change, it lands the way every Coach change lands: as an **Action Proposal** the athlete taps — and a whole week is now one of those, through the same `propose_week_plan` tool. **The server picks the week the proposal may cover, never the model:** a draft the athlete brought in from the calendar → that draft's week; a change to the week they are in → the remainder of this week; nothing else. The prompt no longer offers "or from next Wednesday?". This reverses `coach-actions/PRD.md`'s "a whole Week Plan is never a Coach Action", which existed only because the Weekly Session was the other path. The **Check-in** is asked by a reminder in the Coach Overlay on the Weekly Session Day — the single sanctioned nudge, repurposed — scheduled before the draft but never forced; a late one is simply the freshest signal for whatever reads it next. The **Presence Arc** is re-keyed on what the Coach actually has (weeks of Session Reflections, Check-ins) rather than on sessions held, so its posture is honest by construction; Mads marks it provisional and the whole Hyper Intelligence question as one to re-establish later.

**Why.** A structured weekly ritual earned its place when it was the only way a week got planned and the only moment form was asked. With those jobs done elsewhere, keeping the room means keeping a second write path for weeks, a second prompt posture, and a "Plan my week" door that leads somewhere the athlete no longer needs to go. One flow, one rule for writes, is smaller and cannot contradict itself — which the old pair already did: the Weekly Session's prompt offered next week and its server refused it (found 2026-09-15).

**Consequences.** `weekly_session` stays as a stored conversation kind for transcripts already written; nothing new is created with it. The "Weekly Session Day" keeps its name for now, meaning the day the week turns over. Built and not yet reworked: `training-architecture/18`'s "discuss" starts a `weekly_session` conversation with the draft staged — under this amendment it should start ordinary chat with the draft as its pending proposal. That, the chat tool, the reminder, the arc's re-keying and the header button's removal are one follow-up slice (`training-architecture/20`), after 16–18 are reviewed and merged.
