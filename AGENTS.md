...
- **When several unrelated routes 404 at once**, read
  [stale-next-cache.md](docs/rules/stale-next-cache.md) — it is the build
  cache, not the code: delete `.next` and restart; a cold dev server also
  shows the not-found page once per uncompiled route, so reload first.
