...
- **No direct identifier is sent to the LLM as a field.** Identity is separated from training data by opaque athlete id (ADR 0006), so no name, email, DOB or location travels as a column into a prompt. Prompt builders additionally assert against *shaped* identifiers — email and phone — in the free text that does reach the model (`assertNoDirectIdentifier`, `src/lib/identifiers.ts`, called from `src/features/coach/prompts.ts` and `briefing.ts`). **That assertion is a backstop over free text, not a guarantee about it**: no pattern recognises a name in prose, so a name an athlete types into a note does reach the model, and is covered by the consent disclosure (`src/features/consent/disclosure.ts`) rather than by a filter. Do not write, or repeat, the claim that a name can never reach the API — that sentence has been wrong five times now (`.scratch/preferred-name/PRD.md`). *(Corrected 2026-09-18, `preferred-name/01`. This bullet read "Name, email, DOB, and location are never sent to the Anthropic API (GDPR decision 1). Prompt builders assert this." — the standard CodeRabbit enforced on PR #36, and an overstatement of what the assertion does.)*
- **Colocate tests** with the code they test (`calc-load.ts` next to `calc-load.test.ts`).

### A primitive ships with its snapshot

A component in `src/components/ui` is covered by a `*.visual.tsx` in that folder (its own file, or a case in `primitives.visual.tsx`), and any change to one — or to `globals.css` — is run through `npm run test:visual` before commit; a full-page change through `npm run test:e2e` (runs on its own Neon branch `test/e2e` — `E2E_DATABASE_URL` in `.env.local`, `neon connection-string test/e2e` — reset to its parent `seed-template` with the Neon CLI and reseeded every run, with the Coach switched off by `COACH_DISABLED=1`; it starts its own dev server on 3001, so stop a hand-started one first). Baselines are committed `-win32` PNGs under `__snapshots__/`, regenerated with the matching `:update` script only when the change in appearance was the point. Rationale and the platform ruling: `.scratch/frontend-quality/`.
